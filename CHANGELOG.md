...
- feat(commands): `bramble diagnostics` surfaces the radio's transmit-path health, the airtime backpressure counters and the GNSS raw-feed counters. Radio health leads with the faults that stop usable transmission, the power amplifier failing to ramp and configuration writes not reaching the chip, and warns separately on a failed synthesizer lock, oscillator start or calibration, which cost link budget without failing a transmit. Chip-specific supporting values print verbatim as a `Detail` line. A target whose driver cannot interrogate its transmit path prints the programmed power and says the rest is unavailable, and sections the node omits are skipped rather than rendered as zeros
