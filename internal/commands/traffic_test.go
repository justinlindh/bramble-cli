...
func TestFormatTrafficEventLine_RendersWhateverAddressArrives(t *testing.T) {
	...
	// The renderer keys presence off the empty string and never inspects the
	// value, so an address reading as all zeros still renders. Firmware will
	// not send this one: it treats a zero origin as its own "carries none"
	// sentinel and omits the key entirely. This pins the renderer against
	// acquiring a second, client-side sentinel.
	line := formatTrafficEventLine(bramble.TrafficEvent{
...
// TestTrafficEventDecodesWireShape drives the renderer from a payload in the
// shape the node sends, rather than from a struct the test filled in itself.
// A test that only builds TrafficEvent values proves the renderer works while
// staying silent if the JSON tag never matches what arrives, which is the one
// way this rendering can be dead on a real node. The literals below mirror
// api/openapi.yaml TrafficEvent and what traffic_event_add_json emits: an
// origin address as 8 uppercase hex digits, and the key omitted entirely on a
// TX event and on packet types that carry no origin.
